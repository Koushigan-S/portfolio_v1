...
function GalaxyCanvas({ selectedId, onSelect }: { selectedId: string | null; onSelect: (id: string) => void }) {
  return (
    <Canvas
    ...
    >
      <GalaxyScene selectedId={selectedId} onSelect={onSelect} />
    </Canvas>
  );
}
  ...
  return (
    <AnimatePresence>
      <div className="fixed inset-0 z-[8000] flex items-center justify-center p-4">
        ...
        <motion.div
          className="relative z-10 w-full max-w-3xl bg-[#0d0d12]/95 border border-blue-500/20 rounded-xl p-8 overflow-y-auto max-h-[85vh] shadow-[0_0_50px_rgba(59,130,246,0.15)]"
      ...
      <div className="container relative z-10">
        ...
        >
          ...
          {experienceConfig.features.projectGalaxy && deviceTier !== 'low' ? (
            <GalaxyCanvasDynamic selectedId={selectedId} onSelect={handleSelect} />
          ) : (
            // High quality fallback list layout
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 p-8 h-full overflow-y-auto">
              {projects.map((p) => (
                <button
                  key={p.id}
                  onClick={() => handleSelect(p.id)}
                  className="p-6 rounded border border-blue-500/10 bg-black/30 hover:bg-[#0d0d12]/50 hover:border-blue-500/30 text-left transition-all duration-300 relative group"
                  data-cursor-magnetic
                >
