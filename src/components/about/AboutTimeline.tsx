        ...
        <div className="relative max-w-3xl mx-auto">
          ...
          {aboutTimeline.map((entry, index) => {
            ...
            return (
              <motion.div
                key={entry.id}
                className={`relative flex mb-8 md:mb-12 ${isEven ? 'md:flex-row' : 'md:flex-row-reverse'} flex-row`}
                initial={shouldReduceMotion ? { opacity: 0 } : { opacity: 0, y: 20 }}
                whileInView={{ opacity: 1, y: 0 }}
              ...
              >
                ...
                <div className={`ml-12 md:ml-0 w-full md:w-1/2 ${isEven ? 'md:pl-12' : 'md:pr-12'}`}>
                  {isClassified ? (
                    <div
                      className="p-[15px] rounded-xl border bg-gradient-to-br from-red-950/20 to-black/80 border-red-500/20 shadow-[0_0_24px_rgba(239,68,68,0.08)] relative overflow-hidden"
                    ...
                    <div
                      className="glass p-[15px] rounded-xl border border-white/10 transition-all duration-300 hover:border-blue-500/35 hover:shadow-[0_0_24px_rgba(59,130,246,0.12)] group relative"
                      data-cursor-magnetic
                    >
